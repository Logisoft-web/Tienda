...
export const db = {
  ...
  sesiones:    Datastore.create({ filename: join(dir, 'sesiones.db'),    autoload: true }),
}
...
async function seed() {
  const superadmin = await db.usuarios.findOne({ rol: 'superadmin' })
  const productosExistentes = await db.productos.count({})
  // Solo correr seed si NO existe superadmin Y NO hay productos — doble protección
  if (!superadmin && productosExistentes === 0) {
