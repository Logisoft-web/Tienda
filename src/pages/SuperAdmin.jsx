...
export default function SuperAdmin() {
  ...
  const cargarSesiones = async () => {
    try {
      const data = await api.superAdminGetSesiones(100)
      setSesiones(data)
    } catch (e) { mostrarMsg(e.message, 'error') }
  }
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setShowSesiones(!showSesiones); if (!showSesiones) cargarSesiones() }}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold bg-gray-100 text-gray-600 hover:bg-gray-200 border border-gray-200">
            🖥️ Accesos
          </button>
      ...
      {/* Tabla de sesiones / accesos */}
      {showSesiones && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mt-6">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-dark">🖥️ Historial de accesos</h2>
            <button onClick={cargarSesiones}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-right py-3 px-4 text-gray-500 font-medium">Fecha / Hora</th>
                </tr>
              </thead>
              <tbody>
                {sesiones.map((s, i) => {
                  // Parsear user-agent para mostrar info legible
                  const ua = s.user_agent || ''
                  const dispositivo = ua.includes('Mobile') || ua.includes('Android') || ua.includes('iPhone')
                    ? '📱 Móvil'
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      <td className="py-3 px-4">
                        <p className="font-semibold text-dark">{s.nombre}</p>
                        <p className="text-xs text-gray-400">@{s.usuario}</p>
                      </td>
                      <td className="py-3 px-4">
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                          s.rol === 'superadmin' ? 'bg-purple-100 text-purple-700' :
                          s.rol === 'admin' ? 'bg-blue-100 text-blue-700' :
                          'bg-gray-100 text-gray-600'
                        }`}>{s.rol}</span>
                      </td>
                      <td className="py-3 px-4">
                ...
                {sesiones.length === 0 && (
                  <tr><td colSpan={5} className="py-8 text-center text-gray-400 text-sm">Sin registros de acceso</td></tr>
                )}
              </tbody>
            </table>
          </div>
