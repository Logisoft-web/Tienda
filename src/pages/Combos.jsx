...
const CATEGORIAS = [
  { value: '1', label: 'Categoria 1 - Sabor (sin precio)' },
  { value: '2', label: 'Categoria 2 - Bebida (con precio, min 2 items)' },
  { value: '3', label: 'Categoria 3 - Adicion (con precio)' },
  { value: '4', label: 'Categoria 4 - Borde (sin precio)' },
]
...
function FormCombo({ initial, productos, onSave, onClose }) {
  ...
  const minItems  = categoria === '2' ? 2 : 1
  ...
  const guardar = async () => {
    if (!categoria)                    { setMsg('Selecciona una categoria'); return }
    if (!nombre.trim())                { setMsg('Nombre requerido'); return }
    if (items.length < minItems)       { setMsg('Agrega al menos ' + minItems + ' producto(s)'); return }
    if (!sinPrecio && (!precio || +precio <= 0)) { setMsg('Precio requerido para esta categoria'); return }
    if (!sinPrecio && +precio < precioSugerido)  {
      setMsg('El precio no puede ser menor a la suma de productos (' + precioSugerido.toLocaleString('es-CO') + ')')
      return
    }
    setLoading(true)
    try {
    ...
    <>
      ...
      <div>
        ...
        {categoria && (
          <p className="text-xs mt-1" style={{ color: 'var(--text-dim)' }}>
            {sinPrecio ? 'Este combo no lleva precio' : 'Este combo requiere precio'}
            {categoria === '2' ? ' - minimo 2 items obligatorios' : ''}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <input value={nombre} onChange={e => setNombre(e.target.value)}
          placeholder="Ej: Combo Chelada + Snack"
          className="focus:outline-none" style={ic}
      ...
      {/* Descripcion */}
      <div>
        <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>Descripcion</label>
      ...
      <div>
        <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-muted)' }}>
          Productos <span style={{ color: 'var(--danger)' }}>*</span>
          {categoria === '2' && <span style={{ color: 'var(--info)' }}> (minimo 2)</span>}
        </label>
      ...
      {/* Precio - solo si la categoria lo requiere */}
      {!sinPrecio && categoria && (
        <div>
          <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>
          ...
          <input type="number" value={precio} onChange={e => setPrecio(e.target.value)}
            ...
            style={{ ...ic, borderColor: precioSugerido > 0 && +precio < precioSugerido ? 'var(--danger)' : 'var(--border)' }}
            onFocus={e => { e.target.style.borderColor = 'rgba(244,98,42,0.5)' }}
            onBlur={e  => { e.target.style.borderColor = 'var(--border)' }} />
          {precioSugerido > 0 && (
            <div className="flex items-center justify-between mt-1">
              <p className="text-xs" style={{ color: +precio < precioSugerido ? 'var(--danger)' : 'var(--text-dim)' }}>
                {+precio < precioSugerido
                  ? 'Precio menor a la suma: $' + precioSugerido.toLocaleString('es-CO')
...
export default function Combos() {
  ...
  const handleEditar = async (data) => {
    await api.updateCombo(seleccionado.id || seleccionado._id, data)
    await cargar()
    setModal(null)
  }
  ...
  const catLabel = (cat) => {
    const c = CATEGORIAS.find(x => x.value === String(cat))
    return c ? c.label.split(' - ')[1] : ''
  }
          ...
          <div key={combo.id || combo._id} className="rounded-2xl p-4"
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {combo.categoria && (
                    <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Cat. {combo.categoria}</p>
                  )}
                </div>
              </div>
              ...
              {(combo.items || []).map((item, i) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  <span style={{ color: 'var(--text-muted)' }}>
                    <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{item.cantidad}x</span> {item.nombre}
                  </span>
            ...
            <div className="flex items-center justify-between pt-2"
              ...
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
                ...
                {(combo.items || []).reduce((s, i) => s + i.cantidad, 0)} productos
              </span>
